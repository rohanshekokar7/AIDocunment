...
def main():
    if len(sys.argv) < 5:
        print("Usage: python worker.py <job_id> <tmp_path> <filename> <all_pages> [doc_category]")
        sys.exit(1)
    ...
    doc_category = sys.argv[5] if len(sys.argv) > 5 else "auto"
    ...
    try:
        ...
        from app.pipeline.stages.local_vision import LocalVisionEngine
        ...
        result = pipeline.process(tmp_path, filename, all_pages, doc_category)
